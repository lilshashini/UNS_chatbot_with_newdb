...
class Config:
    ...
    MAX_RESULTS = 1000
    ALLOWED_TABLES = [
         "sites", "areas", "production_lines", "assets",
        "asset_edge_data", "asset_line_data", "asset_unified_data", "asset_dispatch",
        "operations_schedule", "erp_orders", "mes_kpis", "quality_control",
        "maintenance_records", "s88_batch_control", "edge_process_variables",
        "iso55001_data", "dashboard_metrics", "pics_data", "iatech_data",
        "bigquery", "bigquery_kpis", "bigquery_erp", "bigquery_quality",
        "bigquery_maintenance", "bigquery_process_variables", "bigquery_iso55001",
        "bigquery_s88"
    ]
...
class SimpleSQLGenerator:
    ...
    def generate_factory_overview_query() -> Dict[str, Any]:
        """Generate factory overview query that works with available data"""
        ...
        SELECT 
            ...
            COALESCE(s.business_unit, 'Manufacturing') AS division,
            COALESCE(s.general_manager, 'Not Assigned') AS general_manager,
            COALESCE(a.area_name, 'General') AS area,
        ...
        FROM sites s
        LEFT JOIN areas a ON s.site_id = a.site_id
        LEFT JOIN production_lines pl ON a.area_id = pl.area_id
        LEFT JOIN (
            SELECT DISTINCT ON (line_id) line_id, availability, quality, performance, oee, timestamp
            FROM mes_kpis 
            ORDER BY line_id, timestamp DESC
        ) mk ON mk.line_id = pl.line_id
        LEFT JOIN (
    ...
    def generate_oee_query(site: str = None, area: str = None, line: str = None, 
        ...
        """Generate clean OEE query using correct schema with proper JOINs"""
        
        where_conditions = []
        
        # Build the proper JOIN chain according to the schema hierarchy
        # mes_kpis -> production_lines -> areas -> sites
        joins = """FROM mes_kpis k
        JOIN production_lines pl ON k.line_id = pl.line_id
        ...
        if area:
            ...
            where_conditions.append(f"LOWER(a.area_name) ILIKE '%{clean_area.lower()}%'")
    ...
    def generate_maintenance_query(site: str = None, machine: str = None, 
        ...
        """Generate maintenance query using correct schema hierarchy"""
        ...
        JOIN production_lines pl ON mr.line_id = pl.line_id
        JOIN areas a ON pl.area_id = a.area_id
        JOIN sites s ON a.site_id = s.site_id
        WHERE {where_clause}
    ...
    def generate_mtbf_query(machine: str = None, site: str = None) -> Dict[str, Any]:
        """Generate MTBF query with proper joins to mes_kpis table"""
        ...
        JOIN areas a ON pl.area_id = a.area_id
        JOIN sites s ON a.site_id = s.site_id
        JOIN mes_kpis k ON mr.line_id = k.line_id
        WHERE {where_clause}
        ORDER BY k.timestamp DESC
        LIMIT 1
        """
    ...
    def generate_mttr_query(machine: str = None, site: str = None) -> Dict[str, Any]:
        """Generate MTTR query with proper joins to mes_kpis table"""
    ...
    def generate_batch_control_query(recipe_filter: str = None, 
        ...
        JOIN production_lines pl ON bc.line_id = pl.line_id
        JOIN areas a ON pl.area_id = a.area_id
        JOIN sites s ON a.site_id = s.site_id
        WHERE {where_clause}
    ...
    def generate_item_rejection_query(time_filter: str = "30 days") -> Dict[str, Any]:
        ...
        JOIN production_lines pl ON qc.line_id = pl.line_id
        JOIN areas a ON pl.area_id = a.area_id
        JOIN sites s ON a.site_id = s.site_id
        WHERE qc.timestamp >= NOW() - INTERVAL '{time_filter}'
        GROUP BY qc.item_number, s.site_name, a.area_name, pl.line_name, qc.rejection_reason
    ...
    def generate_quality_query(site: str = None, area: str = None, time_filter: str = "24 hours") -> Dict[str, Any]:
        """Generate quality control query using correct schema hierarchy"""
...
class ImprovedNLToSQLTranslator:
    ...
    def _build_system_prompt(self) -> str:
        ...
        key_tables = [
            'sites', 'areas', 'production_lines', 'assets', 'asset_unified_data', 'erp_orders', 'mes_kpis', 'quality_control', 'maintenance_records', 'dashboard_metrics'
...
- sites: site_id (PK), site_name, business_unit, general_manager
- areas: area_id (PK), site_id (FK to sites), area_name
- production_lines: line_id (PK), area_id (FK to areas), line_name
- assets: asset_id (PK), line_id (FK to production_lines), asset_name, asset_type
- mes_kpis: kpi_id (PK), line_id (FK to production_lines), availability, quality, performance, oee, teep, mttr, mtbf, timestamp
- quality_control: quality_id (PK), line_id (FK to production_lines), order_number, item_number, inspection_result, rejection_reason, rejection_quantity, accepted_quantity, timestamp
- maintenance_records: maintenance_id (PK), line_id (FK to production_lines), machine_id, maintenance_status, last_maintenance_date, next_maintenance_date, maintenance_history, timestamp
- erp_orders: order_id (PK), line_id (FK to production_lines), order_number, order_status, scheduled_start_time, scheduled_end_time, actual_start_time, actual_end_time, produced_quantity, remaining_quantity, item_number, bom, item_description, available_quantity, reserved_quantity, ordered_quantity, location, timestamp

TABLE SELECTION GUIDE:
- "soda recipe", "batch control", "production parameters" → s88_batch_control table
- "OEE", "availability", "quality", "performance" → mes_kpis table  
- "maintenance", "MTBF", "MTTR" → maintenance_records (+ mes_kpis for metrics)
- "quality control", "rejection", "inspection" → quality_control table
...
2. To filter by site name: JOIN areas->sites and use sites.site_name
3. To filter by area name: JOIN areas and use areas.area_name  
4. All data tables (mes_kpis, quality_control, etc.) connect via line_id to production_lines
...
2. Always JOIN through the hierarchy: sites->areas->production_lines->data_tables
...
- Use proper hierarchy: sites->areas->production_lines with LEFT JOIN to all operational tables
...
JOIN areas a ON pl.area_id = a.area_id
JOIN sites s ON a.site_id = s.site_id
WHERE LOWER(s.site_name) ILIKE '%biyagama%' AND LOWER(a.area_name) ILIKE '%press%'
...
3. Manufacturing KPIs (OEE, availability, quality) come from mes_kpis table
4. Maintenance data comes from maintenance_records table
5. Quality control data comes from quality_control table
...
class QueryExecutor:
    ...
    def get_schema_info(self) -> Dict[str, List[str]]:
        """Get database schema information - Updated to match correct schema structure"""
        if self._schema_cache:
            return self._schema_cache
        
        try:
            # Correct schema based on the mandatory rules provided
            schema = {
            # Enterprise Hierarchy (Mandatory Schema)
            'sites': ['site_id', 'site_name', 'business_unit', 'general_manager'],
            'areas': ['area_id', 'site_id', 'area_name'], 
            'production_lines': ['line_id', 'area_id', 'line_name'],
            'assets': ['asset_id', 'line_id', 'asset_name', 'asset_type'],
            
            # Operational & Sensor Data (Mandatory Schema)
            'mes_kpis': ['kpi_id', 'line_id', 'availability', 'quality', 'performance', 'oee', 'teep', 'mttr', 'mtbf', 'timestamp'],
            'quality_control': ['quality_id', 'line_id', 'order_number', 'item_number', 'inspection_result', 'rejection_reason', 'rejection_quantity', 'accepted_quantity', 'timestamp'],
            'maintenance_records': ['maintenance_id', 'line_id', 'machine_id', 'maintenance_status', 'last_maintenance_date', 'next_maintenance_date', 'maintenance_history', 'timestamp'],
            'erp_orders': ['order_id', 'line_id', 'order_number', 'order_status', 'scheduled_start_time', 'scheduled_end_time', 'actual_start_time', 'actual_end_time', 'produced_quantity', 'remaining_quantity', 'item_number', 'bom', 'item_description', 'available_quantity', 'reserved_quantity', 'ordered_quantity', 'location', 'timestamp'],
            'edge_process_variables': ['process_id', 'asset_id', 'spindle_speed', 'feed_rate', 'tool_wear', 'vibration', 'power_consumption', 'tool_change_count', 'material_temperature', 'coolant_temperature', 'part_dimensions', 'surface_finish', 'timestamp'],
            
            # Additional operational data tables
            'iso55001_data': ['iso_id', 'line_id', 'asset_lifecycle_status', 'maintenance_schedule', 'risk_level', 'mitigation_plan', 'oee', 'mtbf', 'mttr', 'regulatory_status', 'last_review_date', 'planned_action', 'timestamp'],
            's88_batch_control': ['batch_id', 'line_id', 'batch_mixing_tank_status', 'bottler_status', 'capper_status', 'temperature_controller', 'volume_control', 'soda_recipe', 'production_parameters', 'operator_interface_status', 'process_data', 'quality_data', 'safety_status', 'raw_json', 'timestamp'],
            'dashboard_metrics': ['dashboard_id', 'line_id', 'oee', 'availability', 'performance', 'quality', 'current_batch_status', 'maintenance_status', 'timestamp'],
            
            # Legacy/backward compatibility tables
            'operations_schedule': ['schedule_id', 'area_id', 'shifts', 'shift_config', 'timestamp'],
            'asset_edge_data': ['edge_id', 'asset_id', 'state', 'outfeed', 'waste', 'infeed', 'total_strokes', 'stroke_raw', 'waste_injected', 'raw_json', 'timestamp'],
            'asset_line_data': ['line_data_id', 'asset_id', 'run_enable', 'outfeed', 'infeed', 'scheduled_rate', 'run_start_time', 'state', 'waste', 'runtime', 'run_id', 'current_production_rate', 'raw_json', 'timestamp'],
            'asset_unified_data': ['unified_id', 'asset_id', 'availability', 'performance', 'quality', 'oee', 'run_time', 'total_time', 'planned_downtime', 'unplanned_downtime', 'total_strokes', 'cycle_time', 'timestamp'],
            'asset_dispatch': ['dispatch_id', 'asset_id', 'run_order_id', 'count_type', 'count_type_name', 'last_count', 'count', 'state_id', 'state_runtime', 'trigger_reason', 'timestamp', 'dispatch_timestamp'],
            'pics_data': ['pics_id', 'device_path', 'parameter_name', 'value', 'timestamp'],
            'iatech_data': ['iatech_id', 'parameter_name', 'value', 'timestamp'],
            }
...
class EnhancedResultFormatter:
    ...
    def _get_factory_data(self, executor: 'QueryExecutor') -> Any:
        """Get comprehensive factory data from database"""
        ...
        ORDER BY 
            s.site_name ASC,
            CASE 
                WHEN COALESCE(a.area_name, '') ILIKE '%press%' THEN 1
                WHEN COALESCE(a.area_name, '') ILIKE '%heat%' THEN 2
                WHEN COALESCE(a.area_name, '') ILIKE '%assembly%' THEN 3
                ELSE 4
            END,
    ...
    def _format_single_record_insights(self, record: Any, question: str) -> str:
        ...
        if any(col in record.index and pd.notna(record[col]) for col in ['order_number', 'item_number', 'produced_quantity']):
            ...
            if 'produced_quantity' in record.index and pd.notna(record['produced_quantity']):
                produced = record['produced_quantity']
                remaining = record.get('remaining_quantity', 0) if pd.notna(record.get('remaining_quantity', 0)) else 0
                if remaining > 0:
        ...
        if any(col in record.index and pd.notna(record[col]) for col in ['maintenance_status', 'inspection_result', 'rejection_quantity']):
            ...
            if 'rejection_quantity' in record.index and pd.notna(record['rejection_quantity']) and record['rejection_quantity'] > 0:
                reject_qty = record['rejection_quantity']
...
class EnhancedVisualizationGenerator:
    ...
    def _has_meaningful_data(df: Any) -> bool:
        ...
        numeric_cols = [col for col in numeric_cols if col not in ['id', 'line_id', 'asset_id', 'site_id', 'area_id']]
    ...
    def _create_time_series_chart(df: Any, question: str, fig, ax):
        ...
        meaningful_numeric_cols = [col for col in numeric_cols if col not in ['id', 'line_id', 'asset_id', 'site_id', 'area_id']]
        ...
        try:
            ...
            if 'oee' in df.columns:
                ...
                if 'produced_quantity' in df.columns:
                    ax.plot(df_sorted['timestamp'], df_sorted['produced_quantity'], 
                           marker='o', linewidth=2, label='Produced Quantity', color='#2E86AB')
                if 'remaining_quantity' in df.columns:
                    ax.plot(df_sorted['timestamp'], df_sorted['remaining_quantity'], 
                ...
                if 'rejection_quantity' in df.columns:
                    ax.plot(df_sorted['timestamp'], df_sorted['rejection_quantity'], 
                           marker='o', linewidth=2, label='Rejected Items', color='#C73E1D')
                if 'accepted_quantity' in df.columns:
                    ax.plot(df_sorted['timestamp'], df_sorted['accepted_quantity'], 
                ...
                numeric_cols = [col for col in numeric_cols if col not in ['id', 'line_id', 'asset_id', 'kpi_id']]
            ...
            ax.xaxis.set_major_locator(mdates.HourLocator(interval=max(1, len(df_sorted)//6)))
    ...
    def _create_distribution_chart(df: Any, question: str, fig, ax):
        ...
        non_null_values = df[main_col].dropna()
...
def main():
    ...
    with col1:
        st.image("Jay_jay_mills_logo.png", width=200)
    with col2:
        st.markdown(
